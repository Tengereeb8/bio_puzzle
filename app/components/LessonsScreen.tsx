...
export default function LessonsScreen({
  ...
  const isCorrect = selected === lesson.correctAnswer;
  ...
  const pick = (i: number) => {
    ...
    if (i === lesson.correctAnswer) {
      setCorrect((n) => n + 1);
      confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } });
      speak("Зөв! Маш сайн!");
    } else {
      setHearts((h) => Math.max(0, h - 1));
      speak("Буруу. Дахин оролдоорой.");
    }
  };
    ...
    <div className="min-h-screen pb-24 bg-linear-to-br from-blue-50 to-purple-50 overflow-auto">
        ...
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className={`w-8 h-8 rounded-lg flex items-center justify-center ${i < hearts ? "bg-red-500" : "bg-gray-200"}`}
              >
                <Heart size={18} fill={i < hearts ? "white" : "none"} color={i < hearts ? "white" : "#9CA3AF"} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 pt-8">
        <div className="mb-6">
          <div className="flex items-start justify-between mb-6">
            <h2 className="flex-1 text-xl font-game-bold">{lesson.question}</h2>
            <button
              onClick={() => speak(lesson.audioText)}
          ...
          {lesson.image && (
            ...
            >
              <div className="text-9xl filter drop-shadow-2xl">{lesson.image}</div>
            </motion.div>
          )}
        </div>
        ...
        <div className="space-y-4">
          {lesson.options.map((option, i) => {
            ...
            const showRight = showFeedback && i === lesson.correctAnswer;
            ...
            return (
              <motion.button
                ...
                className={`w-full p-5 rounded-2xl border-3 text-left shadow-lg transition-all ${
                  showRight ? "bg-green-100 border-green-500"
              ...
              >
                <span className={`text-lg font-game-bold ${showRight ? "text-green-700" : showWrong ? "text-red-700" : "text-gray-800"}`}>
                  {option}
                </span>
              </motion.button>
            );
      ...
      <AnimatePresence>
        {showFeedback && (
          ...
          >
            <div className="max-w-2xl mx-auto p-6 flex items-start justify-between gap-4">
              <div className="flex-1 text-white">
                <h3 className="text-2xl mb-3 font-game-black">
                  {isCorrect ? "Гайхалтай! 🎉" : "Буруу байна!"}
                </h3>
                <p className="text-lg leading-relaxed font-game">{lesson.explanation}</p>
              </div>

              <motion.button
                onClick={next}
                className={`px-8 py-4 text-white rounded-2xl shadow-2xl flex items-center gap-3 ${
                  isCorrect ? "bg-green-600 hover:bg-green-700" : "bg-gray-600 hover:bg-gray-700"
                }`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                <span className="text-lg font-game-bold">{isLast ? "Дуусгах" : "Дараах"}</span>
                {isLast ? <Trophy size={24} /> : <ArrowRight size={24} strokeWidth={3} />}
              </motion.button>
            </div>
          </motion.div>
        )}
