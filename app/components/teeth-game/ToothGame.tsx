...
export default function ToothGame() {
  ...
  const start = useCallback(() => setScreen(mode === "quiz" ? "quiz" : "label"), [mode]);
  ...
  return (
    <div className="w-full max-w-xl bg-white rounded-2xl shadow-md p-6">
      {screen === "home" && <HomeScreen selectedMode={mode} onSelectMode={setMode} onStart={start} />}
      {screen === "quiz" && <QuizScreen questions={questions} onComplete={quizDone} />}
      {screen === "label" && <LabelScreen key={labelKey} parts={labelParts} onComplete={labelDone} />}
      {screen === "result" && (
        <ResultScreen score={result.score} total={result.total} mode={mode} funFact={funFact} onPlayAgain={replay} />
      )}
    </div>
  );
}
